...
namespace Nop.Web.Controllers
{
    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        public virtual async System.Threading.Tasks.Task<IActionResult> OpcCompleteRedirectionPaymentAsync(string data = "", bool paymentStatus = false, int CreditType = 1)
        {
            try
            {
                HesabePaymentResponse paymentResponse = null;
                Store currentStore = _storeContext.CurrentStore;
                ...
                if (!string.IsNullOrWhiteSpace(data))
                {
                    ...
                    var hesabeCrypt = new HesabeCrypt(HesabeConstants.MERCHANT_KEY, HesabeConstants.MERCHANT_IV);
                    ...
                    paymentResponse = JsonConvert.DeserializeObject<HesabePaymentResponse>(dataDecrypted);
                }
                ...
                //get the order
                var order = _orderService.SearchOrders(storeId: currentStore.Id,
                ...
                var paymentMethod = _paymentPluginManager
                    .LoadPluginBySystemName(order.PaymentMethodSystemName, currentCustomer, currentStore.Id);
                if (paymentMethod == null)
                    return RedirectToRoute("Homepage");
                if (!paymentMethod.RecurringPaymentType.ToString().ToLower().Contains("manual"))
                    return RedirectToRoute("Homepage");
                ...
                //_paymentService.PostProcessPayment(postProcessPaymentRequest);
                #region Redirect to Hesabe Payment page
                int status = 1;
                if (string.IsNullOrWhiteSpace(data) && paymentMethod.PaymentMethodDescription.ToLower().Contains("credit"))
                {
                    //_paymentService.PostProcessPayment(postProcessPaymentRequest);
                    HesabeCheckoutData paymentData = new HesabeCheckoutData();
                    // Fill the payment data class object with the configuration values from the merchant profile
                    paymentData.MerchantCode = HesabeConstants.MERCHANT_CODE;
                    paymentData.FailureUrl = HesabeConstants.FAILURE_URL;
                    ...
                    var checkoutApiEncryptedResponse = await HesabeApiHelper.PostCheckoutApi(encryptedPaymentData);

                    // Decrypt the response from the checkout API
                    var checkoutApiDecryptedResponse = hesabeCrypt.Decrypt(checkoutApiEncryptedResponse.ToString());
                    ...
                    return Redirect($"{HesabeConstants.PAYMENT_URL}?data={checkoutApiResponse.Response.Data}");
                }
                else
                {
                    if (paymentStatus)
